...
export function IndexForm() {
	...
	const { setLoading, setData, values, setValue, search, setSearch } = useContext(FetchContext);
	...
	return (
		<Container className="pt-0">
			<div
				className="mx-auto flex flex-1 w-full flex-grow rounded border-2 transform"
				style={isFocused ? { borderColor: '#5a67d8' } : { borderColor: '#dae1e7' }}
			>
				<form className="h-full flex flex-no-wrap flex-1 p-4 bg-indigo-50 " onSubmit={handleSubmit}>
					<div className="h-full flex justify-center text-3xl self-center items-center text-indigo-600 ">
						{' '}
						<GoSearch />
					</div>
					...
					<ButtonPrimary className="transform">
						<TextPrimary>Submit</TextPrimary>
					</ButtonPrimary>
				</form>
			</div>
		</Container>
